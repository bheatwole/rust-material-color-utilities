use crate::utils::{
    color_utils::{argb_from_linrgb, argb_from_lstar, y_from_lstar},
    math_utils::{matrix_multiply, sanitize_degrees_double, signum},
};

use super::{cam16::Cam16, viewing_conditions};
...
/// Checks if a number is bounded between 0.0 and 100.0 (inclusive).
///
/// # Arguments
///
...
/// # Returns
///
/// `true` if the number is bounded, `false` otherwise.
...
fn nth_vertex(y: f64, n: usize) -> [f64; 3] {
    ...
    if n < 4 {
        let g = coord_a;
        let b = coord_b;
        let r = (y - g * Y_FROM_LINRGB[1] - b * Y_FROM_LINRGB[2]) / Y_FROM_LINRGB[0];
        if is_bounded(r) {
            return [r, g, b];
        } else {
            return [-1.0, -1.0, -1.0];
        ...
        let b = coord_a;
        let r = coord_b;
        let g = (y - r * Y_FROM_LINRGB[0] - b * Y_FROM_LINRGB[2]) / Y_FROM_LINRGB[1];
        if is_bounded(g) {
            return [r, g, b];
        } else {
            return [-1.0, -1.0, -1.0];
        ...
        let r = coord_a;
        let g = coord_b;
        let b = (y - r * Y_FROM_LINRGB[0] - g * Y_FROM_LINRGB[1]) / Y_FROM_LINRGB[2];
        if is_bounded(b) {
            return [r, g, b];
        } else {
            return [-1.0, -1.0, -1.0];
...
/// # Returns
///
/// A list of two sets of linear RGB coordinates, each
...
/// Calculates the midpoint between two arrays of numbers.
///
/// # Arguments
///
/// * `a` - The first array.
...
/// Calculates the critical plane below a given number.
///
/// # Arguments
///
/// * `x` - The input number.
...
/// # Returns
///
/// The critical plane below `x`.
fn critical_plane_below(x: f64) -> i32 {
    (x - 0.5).floor() as i32
}

/// Calculates the critical plane above a given number.
///
/// # Arguments
///
/// * `x` - The input number.
...
/// Calculates the inverse chromatic adaptation of a given number.
///
/// # Arguments
///
/// * `adapted` - The input number.
///
/// # Returns
///
/// The inverse chromatic adaptation of `adapted`.
fn inverse_chromatic_adaptation(adapted: f64) -> f64 {
    let adapted_abs = adapted.abs();
    let base = f64::max(0.0, 27.13 * adapted_abs / (400.0 - adapted_abs));
...
fn find_result_by_j(hue_radians: f64, chroma: f64, y: f64) -> u32 {
    ...
    let mut j = (y.sqrt() * 11.0).round();
    // ===========================================================
    // Operations inlined from Cam16 to avoid repeated calculation
    // ===========================================================
    let viewing_conditions = viewing_conditions::default();
    let t_inner_coeff = 1.0 / (1.64 - 0.29_f64.powf(viewing_conditions.n)).powf(0.73);
    let e_hue = 0.25 * (hue_radians.cos() + 2.0).cos() + 3.8;
...
static SCALED_DISCOUNT_FROM_LINRGB: [[f64; 3]; 3] = [
    [
...
static LINRGB_FROM_SCALED_DISCOUNT: [[f64; 3]; 3] = [
    [1373.2198709594231, -1100.4251190754821, -7.278681089101213],
    [-271.815969077903, 559.6580465940733, -32.46047482791194],
    [1.9622899599665666, -57.173814538844006, 308.7233197812385],
];

static Y_FROM_LINRGB: [f64; 3] = [0.2126, 0.7152, 0.0722];
